...
#ANSI Sequences FORM: \033[XXXm
ansi_server="\033[1;92m\t"
ansi_stop="\033[0m"
ansi_left="\033[5;91m\t"
...
def receive_message(client):
	while True:
		try:
			...
			#Format message
			message = f"{ansi_server}{name}: {message}{ansi_stop}".encode(ENCODER)
			broadcast(message)
		except:
			#Get name of client
			index = client_socket_list.index(client)
			...
			broadcast(f"{ansi_left}{name} has left the server{ansi_stop}".encode(ENCODER))
			break
...
def connect_client():
	while True:
		try:
			...
			broadcast(f"{ansi_server}Everyone, {client_name} has joined the chat room...{ansi_stop}\n".encode(ENCODER))
